...
export default function ErrorPage({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => { console.error('[WokGen] Page error:', error); }, [error]);
  return (
    <div style={{ padding: '4rem 2rem', textAlign: 'center' }}>
      ...
      <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'center' }}>
        ...
        <a href="/dashboard" style={{ padding: '0.5rem 1.25rem', background: 'var(--surface-raised, rgba(255,255,255,0.04))', border: '1px solid var(--border)', borderRadius: '6px', color: 'var(--text-muted)', fontSize: '0.875rem', textDecoration: 'none' }}>
          Dashboard
        </a>
      </div>
    </div>
