...
export async function POST(req: NextRequest) {
  try {
    ...
    try { body = await req.json() as Record<string, unknown>; } catch { return API_ERRORS.BAD_REQUEST('Invalid JSON'); }
    if (!body || !(body.prompt as string)?.trim()) return API_ERRORS.BAD_REQUEST('prompt is required');
    ...
    log.error({ err }, 'POST /api/tools/text-to-3d failed');
...
export async function GET(req: NextRequest) {
  try {
    ...
    log.error({ err }, 'GET /api/tools/text-to-3d failed');
    return API_ERRORS.INTERNAL();
  }
}
