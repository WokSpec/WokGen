    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.625rem 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <button onClick={() => copy(value, label)} style={{ background: 'none', border: 'none', color: copied === label ? '#34d399' : 'var(--text-muted)', cursor: 'pointer', fontSize: '0.75rem' }}>{copied === label ? 'Copied' : 'Copy'}</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="tool-page-root">
      <div className="tool-page-header">
        <h1 className="tool-page-title">Timestamp Converter</h1>
        <p className="tool-page-desc">Convert between Unix timestamps, ISO 8601, UTC, and local time.</p>
      </div>
        ...
        <div style={{ border: '1px solid var(--border)', borderRadius: '10px', padding: '0 1rem', background: 'rgba(255,255,255,0.02)' }}>
          ...
          {fromTs && !isNaN(fromTs.getTime()) ? (
            <>
              ...
              <Row label="Local" value={fromTs.toLocaleString()} />
              <Row label="Date only" value={fromTs.toISOString().split('T')[0]} />
              <Row label="Unix (ms)" value={String(fromTs.getTime())} />
            </>
          ...
          {fromDate && !isNaN(fromDate.getTime()) && (
            <>
              ...
              <Row label="Unix (seconds)" value={String(Math.floor(fromDate.getTime() / 1000))} />
              <Row label="Unix (ms)" value={String(fromDate.getTime())} />
